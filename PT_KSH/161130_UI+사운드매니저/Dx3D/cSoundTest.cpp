...
void cSoundTest::render(void)
{
}

//사운드 추가(키값, 파일이름, BGM?, 루프냐?)
void cSoundTest::addSound(std::string keyName, std::string soundName, bool bgm, bool loop)
{
	if (loop)//브금 or 이펙트
	{
		if (bgm)
...
void cSoundTest::play(std::string keyName, float volume)
{
	int count = 0;
...
//사운드 정지
void cSoundTest::stop(std::string keyName)
{
	int count = 0;
	arrSoundIter iter = _mTotalSounds.begin();
...
//사운드 일시정지
void cSoundTest::pause(std::string keyName)
{
	int count = 0;
	arrSoundIter iter = _mTotalSounds.begin();
	for (iter; iter != _mTotalSounds.end(); ++iter, count++)
...
//사운드 다시 재생
void cSoundTest::resume(std::string keyName)
{
	int count = 0;
	arrSoundIter iter = _mTotalSounds.begin();
	for (iter; iter != _mTotalSounds.end(); ++iter, count++)
...
//플레이 중이냐?
bool cSoundTest::isPlaySound(std::string keyName)
{
	bool isPlay;
	int count = 0;
	arrSoundIter iter = _mTotalSounds.begin();
...
//일시정지 중이냐?
bool cSoundTest::isPauseSound(std::string keyName)
{
	bool isPause;
	int count = 0;
	arrSoundIter iter = _mTotalSounds.begin();
